require('app/styles/oldeditor/campaign/campaign-editor-view.sass')
...
module.exports = class CampaignEditorView extends RootView
  ...
  template: require 'templates/oldeditor/campaign/campaign-editor-view'
  className: 'editor'
  ...
  onCampaignLevelClicked: (levelOriginal) ->
    ...
    if key.ctrl or key.command
      url = "/oldeditor/level/#{levelTreema.data.slug}"
...
class LevelNode extends TreemaObjectNode
  ...
  buildValueForDisplay: (valEl, data) ->
    ...
    valEl.append $("<a href='/oldeditor/level/#{_.string.slugify(data.name)}' class='spr'>(e)</a>")
...
addAchievementEditorLink = (node, valEl, achievementId) ->
  ...
  anchor.on 'click', (event) ->
    childWindow = window.open("/oldeditor/achievement/#{achievementId}", achievementId, 'width=1040,height=900,left=1600,top=0,location=1,menubar=1,scrollbars=1,status=0,titlebar=1,toolbar=1', true)
