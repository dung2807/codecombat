require('app/styles/oldeditor/achievement/edit.sass')
RootView = require 'views/core/RootView'
template = require 'templates/oldeditor/achievement/edit'
...
nodes = require 'views/oldeditor/level/treema_nodes'
  ...
  saveAchievement: (e) ->
    ...
    res.success =>
      if window.achievementSavedCallback
        ...
        url = "/oldeditor/achievement/#{@achievement.get('slug') or @achievement.id}"
    ...
    $.ajax
      ...
      success: ->
        noty
          ...
          layout: 'topCenter'
        _.delay ->
          application.router.navigate '/oldeditor/achievement', trigger: true
        , 500
      error: (jqXHR, status, error) ->
        console.error jqXHR
        timeout: 5000
