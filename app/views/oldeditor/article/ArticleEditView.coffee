require('app/styles/oldeditor/article/edit.sass')
RootView = require 'views/core/RootView'
VersionHistoryView = require './ArticleVersionsModal'
template = require 'templates/oldeditor/article/edit'
...
module.exports = class ArticleEditView extends RootView
  ...
  openPreview: ->
    if not @preview or @preview.closed
      @preview = window.open('/oldeditor/article/preview', 'preview', 'height=800,width=600')
  ...
  saveNewArticle: (e) ->
    ...
    res.success =>
      @article.clearBackup()
      modal.modal('hide')
      url = "/oldeditor/article/#{@article.get('slug') or @article.id}"
